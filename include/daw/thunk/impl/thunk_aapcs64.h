...
namespace daw::thunk_impl {
	 ...
	 */
		...
		static constexpr void set_imm_mov( std::uint64_t value,
		                                   arm64_op &op_codes ) {
			...
			if constexpr( off == 0 ) {
				...
				auto const fp32 =
				  static_cast<std::uint32_t>( ( value >> 32 ) & 0xFFFFU );
				...
				auto const fp48 =
				  static_cast<std::uint32_t>( ( value >> 48 ) & 0xFFFFU );
		...
		struct alignas( 16 ) thunk {
			...
			constexpr thunk( std::index_sequence<Is...> ) noexcept
			  : op_codes{
			      op_mov<Is>( )...,      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
			      /*br x16*/ 0xD61F'0200 } {}
